...
//
	...
	// create a temp object to fire at a later time
		...
		if (!activator) 
			gi.dprintf ("Think_Delay with no activator, model: %s target: %s\n", ent->model, ent->target);
...
void	G_TouchTriggers (edict_t *ent)
{
	...
	num = gi.BoxEdicts (ent->absmin, ent->absmax, 
		touch, MAX_EDICTS, AREA_TRIGGERS);
	
	if ((!(strcmp(ent->classname, "angel of life"))) && num > 0)
	{
		ent->goalentity = NULL;
		ent->enemy = NULL;
		ent = ent->owner;
	...
	if ((!(strcmp(ent->classname, "angel of frost"))) && num > 0)
	{
		ent->goalentity = NULL;
		ent->enemy = NULL;
		ent = ent->owner;
	...
	if ((!(strcmp(ent->classname, "angel of plague"))) && num > 0)
	{
		ent->goalentity = NULL;
		ent->enemy = NULL;
		ent = ent->owner;
	}
	if ((!(strcmp(ent->classname, "angel of flame"))) && num > 0)
	{
		ent->goalentity = NULL;
		ent->enemy = NULL;
		ent = ent->owner;
	...
	if ((!(strcmp(ent->classname, "angel of blindness"))) && num > 0)
	{
		ent->goalentity = NULL;
		ent->enemy = NULL;
		ent = ent->owner;
	...
	if ((!(strcmp(ent->classname, "angel of horror"))) && num > 0)
	{
		ent->goalentity = NULL;
		ent->enemy = NULL;
		ent = ent->owner;
...
{ 
	...
	for (j=0; j<3; j++) 
		eorg[j] = (float) abs ((int)(start[j] - end[j])); 
