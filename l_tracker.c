	...
	{ 	
		...
		if (Q_stricmp (ent->trackerammotype, "Blaster") == 0) 
			...
			fire_grenade_dM (ent->owner, 
				...
				QFALSE, // held
				QFALSE); // bazookad		
		else if (Q_stricmp (ent->trackerammotype, "Sticking Grenades") == 0) 
			fire_stickinggrenade (ent->owner, ent->s.origin, ent->movedir, 120, 600, 2.5, 40);
		else if (Q_stricmp (ent->trackerammotype, "Buckyball Launcher") == 0) 
			fire_buckyball (ent->owner, ent->s.origin, ent->movedir, BUCKY_DAMAGE, BUCKY_SPEED, 90);
		else if (Q_stricmp (ent->trackerammotype, "Bazooka") == 0) 
			fire_grenade_dM (ent->owner, 
				ent->s.origin, 
				ent->movedir, 
				120, //damage
				...
				QFALSE, // held
				QTRUE); // bazookad
		else if (Q_stricmp (ent->trackerammotype, "Double Impact") == 0)
		{
		...
		else if (Q_stricmp (ent->trackerammotype, "Rocket Launcher") == 0)
			fire_flamerocket (ent->owner, ent->s.origin, ent->movedir, (100 + (int)(random() * 20.0)), 650, 120, 120);
		else if (Q_stricmp (ent->trackerammotype, "Lightning Gun") == 0)
			fire_lightning (ent->owner, ent->s.origin, ent->movedir, 30);
		else if (Q_stricmp (ent->trackerammotype, "Positron") == 0)
		{
			PositronBeamFire(ent->owner, ent->s.origin, ent->movedir);
			ent->nextthink = level.time + 1.0f;	
		}
		else if (Q_stricmp (ent->trackerammotype, "Freezatron") == 0)
		{
			FreezatronBeamFire(ent->owner, ent->s.origin, ent->movedir);
			ent->nextthink = level.time + 1.0f;	
		}
		else if (Q_stricmp (ent->trackerammotype, "HyperBlaster") == 0)
			fire_blaster (ent->owner, ent->s.origin, ent->movedir, 15, 1000, EF_HYPERBLASTER /* , hyper */);
		else if (Q_stricmp (ent->trackerammotype, "Icecube Gun") == 0)
			fire_freezer (ent->owner, ent->s.origin, ent->movedir, FREEZER_DAMAGE, FREEZER_SPEED, EF_BLASTER);
		else if (Q_stricmp (ent->trackerammotype, "Super Blaster") == 0)
			fire_super (ent->owner, ent->s.origin, ent->movedir, 50, 1000, EF_BLASTER /* , hyper */);
		else if (Q_stricmp (ent->trackerammotype, "Railgun2") == 0)
			fire_rail (ent->owner, ent->s.origin, ent->movedir, 150, 8);
		else if (Q_stricmp (ent->trackerammotype, "Wall Piercing Railgun") == 0)
			fire_rail (ent->owner, ent->s.origin, ent->movedir, 150, 8);
		else if (Q_stricmp (ent->trackerammotype, "Railgun") == 0)
		{
			vec3_t temp = {6, 9, 25};
			vec3_t temp2 = {6, 4, 25};
			PBM_FireFlamer (ent->owner, ent->s.origin, ent->movedir, 1200, 70, temp ,temp2 , 100, 50);
		}
		else if (Q_stricmp (ent->trackerammotype, "Durg") == 0)
			fire_durg (ent->owner, ent->s.origin, ent->movedir, 0, 2);		
		else if (Q_stricmp (ent->trackerammotype, "Energy Vortex") == 0)
			fire_energyvortex (ent->owner, ent->s.origin, ent->movedir, 200, 500, 1000);
		else if (Q_stricmp (ent->trackerammotype, "BFG10K") == 0)
			fire_bfg (ent->owner, ent->s.origin, ent->movedir, 200, 400, 1000);
		else if (Q_stricmp (ent->trackerammotype, "FBFG") == 0)
			fire_fbfg (ent->owner, ent->s.origin, ent->movedir, 200, 400, 1000);
		else if (Q_stricmp (ent->trackerammotype, "Sniper Gun") == 0)
			fire_sniper (ent->owner, ent->s.origin, ent->movedir, 300, 2500, EF_BLASTER);
		else if (Q_stricmp (ent->trackerammotype, "Snipersweeper") == 0)
			fire_sniper (ent->owner, ent->s.origin, ent->movedir, 300, 2500, EF_BLASTER);
		else if (Q_stricmp (ent->trackerammotype, "Plasma Rifle") == 0)
			fire_plasma (ent->owner, ent->s.origin, ent->movedir, 15, 1200, EF_BFG);
		
		else if (Q_stricmp (ent->trackerammotype, "Explosive Chaingun") == 0)
			fire_explosive_bullet (ent->owner, ent->s.origin, ent->movedir, 6, 7, 300,500, MOD_EXPLOSIVECHAINGUN);
		else if (Q_stricmp (ent->trackerammotype, "Explosive Super Shotgun") == 0)
		{
		...
		else if (Q_stricmp (ent->trackerammotype, "Explosive Shotgun") == 0)
		...
		else if (Q_stricmp (ent->trackerammotype, "Explosive Streetsweeper") == 0)
...
void Tracker_Weapon_Fire (edict_t *ent)
{
	...
	//STOP_PERFORMANCE_TIMER(__FUNCTION__);
