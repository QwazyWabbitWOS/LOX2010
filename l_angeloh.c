...
void AOH_Think (edict_t *ent)
{
	...
	if (ent->owner->client->angel != ent || ent->owner->client->pers.special != AOHORROR)
	{
		G_FreeEdict (ent);
		return;
	}
	...
	while ((blip = findradius (blip, ent->s.origin, 100)) != NULL)
	{
		...
		if (blip->client->horrortime > level.time)
			continue;
	...
	if (ent->enemy != NULL && ent->enemy->client == NULL) 
		ent->enemy = NULL;
	else 
	{
		if (ent->enemy != NULL && ent->enemy->client->horrortime > level.time) 
			ent->enemy = NULL;
	}
	
	if (ent->enemy != NULL)
	{
		ent->enemy->client->horrortime = level.time + 10;
