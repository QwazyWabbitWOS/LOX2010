...
#include "g_local.h"
#include "l_angels.h"
#include "l_deathray.h"

static void deathray_touch(edict_t* self, edict_t* other, cplane_t* plane, csurface_t* surf);
...
void Deathray_Fire(edict_t* ent, vec3_t g_offset, int damage, qboolean hyper, int effect)
{
	...
	// tell it to fire the deathray instead
	fire_deathray(ent, start, forward, damage, DEATHRAY_SPEED, effect);
	if (ent->client && ent->client->pers.special == AODEATH)
	{
		fire_deathray(ent, ent->client->angel->s.origin, forward, damage, DEATHRAY_SPEED, effect);
	}
	// send muzzle flash
...
void Weapon_Deathray_Fire(edict_t* ent)
{
	...
	damage = DEATHRAY_DAMAGE;

	Deathray_Fire(ent, vec3_origin, damage, false, EF_BLASTER);
...
void Weapon_Deathray(edict_t* ent)
{
	...
	Weapon_Generic(ent, 6, 17, 57, 61, pause_frames, fire_frames, Weapon_Deathray_Fire);
}
