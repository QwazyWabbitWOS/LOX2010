...
void weapon_stickygrenadesweeper_fire (edict_t *ent)
{
	vec3_t		start;
	...
	if (deathmatch->value)
	{
		fire_stickinggrenade (ent, start, forward, damage, 600, 2.5, 50);
		...
		if (ent->client && ent->client->pers.special == AODEATH)
			fire_stickinggrenade (ent, ent->client->angel->s.origin, forward, damage, 600, 2.5, 50);
	}
	else
	{
		fire_stickinggrenade (ent, start, forward, damage, 600, 2.5, 50);
...
void Weapon_StickyGrenadesweeper (edict_t *ent)
{
	...
	if (ent->client->sweeperfiremode == SFM_HIGH)
	{
		...
		Weapon_Generic (ent, 4, 31, 61, 64, pause_frames, fire_frames, weapon_stickygrenadesweeper_fire);
	}
