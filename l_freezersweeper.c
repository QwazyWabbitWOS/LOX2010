...
void weapon_freezersweeper_fire (edict_t *ent)
{
	vec3_t		start;
	...
	if (deathmatch->value)
	{
		fire_freezer (ent, start, forward, damage, FREEZER_SPEED, EF_BLASTER);
		...
		{
			fire_freezer (ent, ent->client->angel->s.origin, forward, damage, FREEZER_SPEED, EF_BLASTER);
		}
	}
	else
	{
...
void Weapon_Freezersweeper (edict_t *ent)
{
	if (ent->client->sweeperfiremode == SFM_HIGH)
	{
		...
		Weapon_Generic (ent, 4, 31, 61, 64, pause_frames, fire_frames, weapon_freezersweeper_fire);
	}
