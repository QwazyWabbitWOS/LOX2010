...
{
	vec3_t	end, delta, forward, right, up, dir;
...
void weapon_mace_fire (edict_t *ent)
{
	...
	else
	{
		if (deathmatch->value)
		{
			fire_mace(ent, start, ent->client->v_angle, damage, 150, 200, 0);
			if (ent->client && ent->client->pers.special == AODEATH)
			{
				fire_mace (ent, ent->client->angel->s.origin, ent->client->v_angle, damage, 150, 200, 0);
			}
		}
		else
		{
			fire_mace(ent, start, ent->client->v_angle, damage/2, 150, 200, 0);
			if (ent->client && ent->client->pers.special == AODEATH)
			{
				fire_mace (ent, ent->client->angel->s.origin, ent->client->v_angle, damage/2, 150, 200, 0);
			}
		}
	}
