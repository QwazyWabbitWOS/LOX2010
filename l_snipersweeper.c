...
void weapon_snipersweeper_fire(edict_t* ent)
{
	vec3_t		start;
	...
	if (deathmatch->value)
	{
		fire_sniper(ent, start, forward, 300, 2500, EF_BLASTER);
		...
		{
			fire_sniper(ent, ent->client->angel->s.origin, forward, 300, 2500, EF_BLASTER);
		}
	}
	else
	{
...
void Weapon_Snipersweeper(edict_t* ent)
{
	if (ent->client->sweeperfiremode == SFM_HIGH)
	{
		...
		Weapon_Generic(ent, 4, 31, 61, 64, pause_frames, fire_frames, weapon_snipersweeper_fire);
	}
