...
qboolean Jet_AvoidGround( edict_t *ent )
{
	...
	success = (qboolean)(trace.plane.normal[2] == 0);
	if ( success )     /*no ceiling?*/
...
void Jet_ApplyLifting( edict_t *ent )
{
	...
	delta = (float) sin( (float)((level.framenum%time)*(360.0f/time))/180*M_PI ) * amplitude;
...
void Jet_ApplyJet( edict_t *ent, usercmd_t *ucmd )
{
	...
	{
		ent->client->Jet_next_think = level.framenum + 1.0f;
		...
		if ( ucmd->forwardmove )
		{
			...
			direction = (ucmd->forwardmove<0) ? -1.0f : 1.0f;
		...
		if ( ucmd->sidemove )
		{
			...
			direction = (ucmd->sidemove<0) ? -1.0f : 1.0f;
		...
		to 0 in a few steps*/
		ent->velocity[0] += -(ent->velocity[0]/6.0f);
		ent->velocity[1] += -(ent->velocity[1]/6.0f);
		ent->velocity[2] += -(ent->velocity[2]/7.0f);
		...
		/*Bound velocities so that friction and acceleration dont need to be
		synced on maxvelocities */
